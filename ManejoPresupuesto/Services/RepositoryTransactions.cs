...
public class RepositoryTransactions : IRepositoryTransactions
{
    ...
    {
        using var connection = new SqlConnection(connectionString);
        return await connection.QueryAsync<TransaccionesSemanal>(
    ...
    {
        using var connection = new SqlConnection(connectionString);
        return await connection.QueryAsync<TransaccionesMensual>("""
                                                                 SELECT MONTH(t.FechaTransaccion) AS Month,
                                                                 SUM(t.Monto) AS Monto, cat.TipoOperacionId
                                                                 FROM Transacciones t
                                                                 INNER JOIN Categorias cat
                                                                 ON cat.Id = t.CategoriaId
                                                                 WHERE t.UsuarioId = @usuarioId AND YEAR(t.FechaTransaccion) = @year
                                                                 GROUP BY t.FechaTransaccion, cat.TipoOperacionId;
                                                                 """,
            new { usuarioId, year });
    }
